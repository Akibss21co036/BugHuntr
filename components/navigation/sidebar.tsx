...
function SidebarComponent({ isOpen = false, onClose }: SidebarProps) {
  ...
  const navigationItems = [
    ...
    { id: "learn", label: "Learn", icon: Brain, href: "https://maaz.great-site.net/?i=1", target: "_blank", rel: "noopener noreferrer" },
    {
      id: "communities",
      label: "Communities",
      icon: Users,
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="px-4 py-4 border-b border-sidebar-border">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-xs font-semibold text-sidebar-foreground/70 uppercase tracking-wider">Communities</h3>
                ...
                {joinedCommunities.map((community) => {
                  ...
                  return (
                    <div key={community.id}>
                      <Button
                        variant="ghost"
                        className={cn("w-full justify-start gap-2 h-8 text-sm px-2", isCurrentCommunity && "bg-sidebar-accent")}
                        onClick={() => {
                        ...
                        {isExpanded && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="ml-6 space-y-1 overflow-hidden">
                            {community.channels.map((channel) => (
                              <Link key={channel.id} href={`/community/${community.id}/channel/${channel.id}`} onClick={onClose}>
                                <Button variant="ghost" className="w-full justify-start gap-2 h-7 text-xs px-2 text-sidebar-foreground/70 hover:text-sidebar-foreground">
            ...
            <nav className="px-4 py-4 space-y-1">
              <h3 className="text-xs font-semibold text-sidebar-foreground/70 uppercase tracking-wider mb-3">Navigation</h3>
                    ...
                    <Link href={item.href} onClick={onClose}>
                      <Button
                        ...
                        className={cn(
                          ...
                          (item as any).isPro && "bg-gradient-to-r from-amber-500/10 to-purple-500/10 border-l-2 border-amber-500"
                        )}
                      >
                        <Icon className={cn("h-4 w-4", (item as any).isPro && "text-amber-500")} />
                        {item.label}
                        {(item as any).isPro && (
                          <span className="ml-auto px-2 py-0.5 text-xs font-bold bg-gradient-to-r from-amber-500 to-yellow-500 text-black rounded-full">PRO</span>
                        )}
                      </Button>
                    </Link>
                  </motion.div>
          ...
          <motion.div className="p-4 border-t border-sidebar-border" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
            <div className="bg-sidebar-accent rounded-lg p-3 space-y-3 overflow-hidden">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 bg-gradient-to-br from-cyber-blue to-neon-green rounded-full flex items-center justify-center flex-shrink-0">
                  <span className="text-white font-bold text-sm">{user?.username ? user.username.charAt(0).toUpperCase() : "JD"}</span>
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 overflow-hidden">
                    {user?.role === "admin" ? (
                      ...
                      <p className="text-xs text-sidebar-foreground/70 truncate">{"@" + (user?.username || "johndoe")}</p>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                ...
                <div className="text-center">
                  <div className="font-semibold text-neon-green">${userRanking ? (userRanking.totalEarnings / 1000).toFixed(1) : "12.4"}K</div>
